...
function InternationalList({ events }) {
  const router = useRouter();

  const resolvedEvents = use(events);

  const internationalEvents = resolvedEvents.filter(
    (event) => event?.type === "international"
  );
  ...
  return (
    <div className="">
      ...
      {internationalEvents.length === 0 &&
        eventCategory === "international" && <NothingToDisplay />}
    </div>
  );
}
